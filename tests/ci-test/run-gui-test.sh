...
if [ -f "/workspace/freecad/RobustMCPBridge/freecad_mcp_bridge/blocking_bridge.py" ]; then
    echo "Starting FreeCAD with MCP bridge..."

    freecad /workspace/freecad/RobustMCPBridge/freecad_mcp_bridge/blocking_bridge.py > /tmp/freecad_bridge.log 2>&1 &
    FREECAD_PID=$!
